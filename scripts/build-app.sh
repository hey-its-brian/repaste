...
cp "$BIN/Repaste" "$APP/Contents/MacOS/"
...
<plist version="1.0">
<dict>
  <key>CFBundleName</key><string>Repaste</string>
  <key>CFBundleDisplayName</key><string>Repaste</string>
  <key>CFBundleIdentifier</key><string>com.heyitsbrian.repaste</string>
...
# (needed for auto-paste) to the signature; an ad-hoc signature changes on every
...
if [[ "${1:-}" == "--install" ]]; then
  rm -rf /Applications/Repaste.app
  cp -R "$APP" /Applications/
  echo "Installed to /Applications/Repaste.app"
fi
